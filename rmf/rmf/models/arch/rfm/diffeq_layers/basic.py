...
class ConcatLinear_v2(nn.Module):
    ...
    def __init__(self, dim_in, dim_out):
        ...
        self._hyper_bias_r = nn.Linear(1, dim_out, bias=False)
        self._hyper_bias_r.weight.data.fill_(0.0)

    def forward(self, r, t, x):
        return (
            self._layer(x)
            + self._hyper_bias(t.view(-1, 1))
            + self._hyper_bias_r(r.view(-1, 1))
        )
...
class TimeDependentSwish(nn.Module):
    ...
    def forward(self, r, t, x):
        ...
        gamma = self.gamma(r.reshape(-1, 1))
        return x * torch.sigmoid_(x * (beta - gamma))
